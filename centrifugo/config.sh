...
finalize_yum_repo ()
{
  if [ "$_skip_pygpgme" = 0 ]; then
    ...
    yum install -y pygpgme --disablerepo='FZambia_centrifugo'
    ...
    if [ "$?" != "0" ]; then
      ...
      sed -i'' 's/repo_gpgcheck=1/repo_gpgcheck=0/' /etc/yum.repos.d/FZambia_centrifugo.repo
    fi
  fi

  echo "Installing yum-utils..."
  yum install -y yum-utils --disablerepo='FZambia_centrifugo'
  ...
  echo "Generating yum cache for FZambia_centrifugo..."
  yum -q makecache -y --disablerepo='*' --enablerepo='FZambia_centrifugo'

  echo "Generating yum cache for FZambia_centrifugo-source..."
  yum -q makecache -y --disablerepo='*' --enablerepo='FZambia_centrifugo-source'
}

finalize_zypper_repo ()
{
  zypper --gpg-auto-import-keys refresh FZambia_centrifugo
  zypper --gpg-auto-import-keys refresh FZambia_centrifugo-source
}
...
main ()
{
  ...
  yum_repo_config_url="https://packagecloud.io/install/repositories/FZambia/centrifugo/config_file.repo?os=${os}&dist=${dist}&source=script"

  if [ "${os}" = "sles" ] || [ "${os}" = "opensuse" ]; then
    yum_repo_path=/etc/zypp/repos.d/FZambia_centrifugo.repo
  else
    yum_repo_path=/etc/yum.repos.d/FZambia_centrifugo.repo
  fi
